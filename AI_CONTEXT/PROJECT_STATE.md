...
- **`Code.gs`:** route ai-chat bọc try/catch → `_aiErrorResponse_` trả lỗi CÓ CẤU TRÚC (errorCode+retriable) + **degradation deterministic** (khi quá tải → trả khối "SỐ LIỆU TÍNH SẴN", chính xác không qua LLM, **KHÔNG rời biên dữ liệu** — loại OpenRouter/bên thứ 3 vì data-boundary ngân hàng).
- **FE `ai-chat.js`+i18n+css:** thông điệp thân thiện theo errorCode + **nút "Thử lại"** (thủ công, không tự đốt quota); FE KHÔNG auto-retry lỗi Gemini (server đã lo). Tương thích ngược 2 chiều GAS cũ/mới.
- **Test live production (đã login):** câu "task Blocked" trả bảng đúng, **latency ~15-20s** (payload ~150-200K token/câu — đòn bẩy dài hạn N2.1 giảm payload). 503 gián đoạn phía Google (2 lần test không trúng; đường code gây lỗi xác nhận chắc bằng code).
- **Verify:** MỚI `test/verify_ai_resilience.mjs` **42/42** (chain·backoff·classify·retry·fallback·404·400·empty·errorResponse·degradation). Regression: my_work 97/97 · startup_nonblocking 10/10 · nav_group 14/14 · parse OK 2 .gs. Blocker: không. **[TT]:** (1) **redeploy GAS** (AiService.gs+Code.gs) — fix THẬT ở server; (2) tùy chọn chạy `refreshAiModelChain()` 1 lần trong editor để bơm chain dự phòng + **bật billing** giảm tần suất 503; (3) hard-refresh `?v=20260904b`.

---

**S83 — CR LEFT-MENU 2 LỚP (gom nhóm menu mẹ, thu gọn sidebar) — CODE XONG + TEST PASS LOCAL. Thuần FE, KHÔNG redeploy GAS. (main, v6.58, 2026-09-04)**
CR [TT]: left menu 28 dòng quá dài → gom tính năng cùng nhóm vào **menu mẹ gấp/mở được**, giữ mục dùng nhiều (My Work) phẳng ở trên. **Kiến trúc mới:** 3 mục phẳng (My Work · Dev-plan · Dashboard) + **5 nhóm mẹ** (Ban lãnh đạo · Quản trị H2 · Quản lý công việc · KPI Digital & Phân tích · Quản trị[admin]) + AI Assistant phẳng. Gấp hết ≈ 28→10 dòng.
- **Hành vi ([TT] chốt 3 lựa chọn):** (1) mặc định mở "Quản lý công việc" + nhóm chứa view active, còn lại gấp — nhớ qua `localStorage` (`shtd_nav_groups`); (2) sidebar thu gọn 68px → **flyout hover** submenu bên phải; (3) **badge dồn** ở nhóm mẹ: chấm đỏ khi nhóm GẤP có badge `danger` (Phê duyệt/Issue/Overdue) ở con → không lỡ việc; nhóm mở → ẩn (con tự hiện).
- **Bảo toàn:** phím tắt `g`+chữ giữ nguyên (gọi thẳng `navigateTo`, độc lập DOM lồng); `.active` theo `[data-view]`; RBAC `admin-only` nguyên; auto-mở nhóm chứa view active. Đổi nhãn 3 mục con H2 (bỏ tiền tố lặp "Quản trị H2 ·" → Dashboard/KPI/Tự đánh giá); bỏ 6 `nav-section-label` phẳng cũ.
- **Files:** `index.html` (tái cấu trúc `<nav>` + bump `?v=20260904` cho 6 file đổi) · `assets/css/layout.css` (nav-group/header/caret/dot/body + flyout collapsed + has-alert, theme-aware) · `assets/js/ui/navigation.js` (`toggleNavGroup`/`applyNavGroupState`/`updateNavGroupBadges`/`_expandGroupOfActive` + persist + wire vào `navigateTo`/`setupListeners`) · `assets/js/i18n.js` (5 key `nav.group.*` VI/EN + rút gọn nhãn H2) · `assets/js/app.js` + `views/issue-tracker.js` (gọi `updateNavGroupBadges` sau cập nhật badge con) · `assets/js/config.js` (v6.58).
- **Verify:** MỚI `test/verify_nav_group.mjs` **14/14** (structure · không mất view 22/22 · mặc định · toggle+persist · reload-persist · auto-expand · nav view trong nhóm · phím tắt g+k · badge dồn · flyout 68px · 0 JS error) + ad-hoc mobile 375px PASS. **Regression:** my_work 97/97 · case_pipeline 22/22 · bld_queue 45/45 · h2_dashboard/review/tracker 24+20+32 · i18n_p8 13/13 · issue_tracker 61/61 · action_plan 24/24 · es_init_health 14/14 · modal_layout 9/9 · filter_cascade 23/23 · atomic 41/41 · notifications 21/21 · recurring 23/23 · startup 10/10 · calendar_sync 32/32. **Đã vá 9 test click nav item trong nhóm gấp** (mở nhóm cha trước click — hành vi đúng: user cũng phải mở nhóm). Pre-existing (không phải regression): `verify_mobile`/`verify_kpi_views` cần server ngoài 53704; `verify_initiative*` flaky file:// (nav initiative-tracker thuộc nhóm "work" mở sẵn, retry timeout ở bước khác qua nav). Blocker: không. **[TT] hard-refresh `?v=20260904` → nghiệm thu.**
...
- **✅ [TT] đã redeploy GAS** (dán 2 .gs + `installCalendarSyncTrigger()`, link không đổi). **Verify:** `verify_calendar_sync` **32/32** (sandbox, logic thuần) + regression `my_work` **97/97** (0 JS error). Phần chạm CalendarApp thật → [TT] smoke live. Thiết kế: `docs/PROPOSAL_Calendar_Sync_2026-09-01.md`.
- **FIX v6.57.1 (3 phản hồi [TT]):** (1) **Theme** — thẻ 🔔 chuyển sang **biến token** (`--surface2`/`--border`/`--text*`/`--success-bg`/`--danger`), bỏ hardcode + `prefers-color-scheme` → đồng bộ sáng/tối như mọi view. (2) **Không bắt nhập email** — bỏ ô input; server lấy **Email đăng ký trong User_Master** (`_calUserEmail`); `calEnable(email?)` fallback User_Master; UI hiện "Sẽ đồng bộ tới email đăng ký: <email>" + nút Kết nối (nếu hồ sơ chưa có Email → báo liên hệ Admin). (3) **Lỗi permission Calendar** — gốc: `installCalendarSyncTrigger()` chỉ dùng `ScriptApp` → owner **chưa cấp scope Calendar**; thêm hàm `authorizeCalendarScope()` (gọi `CalendarApp.getDefaultCalendar()`) → **[TT] chạy 1 lần trong editor để Authorize** + redeploy New version. `?v=20260901b`, v6.57.1. Verify lại 32/32 + 97/97. **✅ LIVE OK (2026-09-01):** [TT] chạy `authorizeCalendarScope()` + Authorize Calendar + redeploy New version → **event đã lên Google Calendar thành công** (smoke live PASS). Tính năng Pha 1 hoàn tất trên production. Blocker permission đóng.
...
> **S80 (Task ĐỊNH KỲ tuần/tháng — FEATURE, log 1 lần + tick mỗi kỳ)**: Yêu cầu [TT] — theo dõi task định kỳ (hàng tuần/tháng) mà **log 1 lần**, mỗi kỳ **tick 1 click**, **tận dụng tối đa nền tảng + ít đụng cấu trúc nhất**. Thiết kế PA3 (tái dùng Report Week membership S62 + hạ tầng noti): **+2 cột APPEND** `Định kỳ` (Z, ''|Tuần|Tháng) + `Kỳ đã xong` (AA, danh sách nhãn kỳ), `taskToRow`/parser/GS_RANGE `A1:AA` (backend GAS đọc/ghi động → **Pha 1 KHÔNG redeploy Web App**). Helper `taskPeriodStatus`/`togglePeriodDone`/`taskTogglePeriodDone`/`taskPeriodBadgeHtml` (helpers.js): trạng thái kỳ **suy ra** (✓ Xong / Chưa / **MISS đỏ** kỳ trước), **auto-reset** mỗi kỳ (0 job). Nút **"✓ Xong kỳ này"** ở **My Work + bảng Tasks + popup chi tiết** (lưu qua route `task-upsert` cũ, 0 route mới) + chip ↻ + select "Định kỳ" trong modal. **Pha 2 (nhắc — CẦN redeploy GAS)**: `NotificationService.gs` sinh noti `recur-miss` — task định kỳ chưa tick **kỳ TRƯỚC** → nhắc ở kỳ hiện tại kèm tên kỳ ("↻ Định kỳ chưa hoàn thành Tuần WW/YYYY"), thu hồi khi tick (reconcile). Migration `RecurrenceMigration.gs` (set Z1/AA1 + backfill, chạy editor). Tests: `verify_recurring` **23/23** · regression my_work 91/91 · report_week 17/17 · atomic_write 41/41 (cập nhật 25→27 cột) · task_rag 5/5 · notif_retract 41/41 · notifications 21/21. **Việc thủ công [TT]:** (1) chạy `commitAddRecurrence()` (GAS editor) tạo 2 cột; (2) redeploy GAS cho Pha 2 nhắc; (3) hard-reload → v6.53. Thiết kế đầy đủ: `AI_CONTEXT/RECURRING_TASK_DESIGN.md`.

> **S79 (Email nhắc việc lệch ngày/tháng + định dạng — DEBUG, backend + test, v6.52 giữ nguyên)**: [TT] báo digest nhắc việc vẫn hiện lẫn "31-Jul-26" (locale Anh) vs "21/08/2026" + nhầm tháng (task hiện quá hạn với ngày cũ dù deadline đã sửa). Gốc trong `backend/NotificationService.gs`: (A) `NotifID` KHÔNG gồm ngày → deadline đổi thì `notifScan` bỏ qua (append idempotent) + `_notifRetractStale_` chỉ thu hồi khi done/mất, không khi deadline dời khỏi ngưỡng; (B) `_notifMessage_` echo `dueStr` thô theo locale ô Sheet. **Fix 3 tầng thuần backend:** NEW `_notifFmtDue()` chuẩn hoá mọi ngày → **DD/MM/YYYY** (dựng thủ công, không `Utilities.formatDate`, parse fail → giữ nguyên); `_notifMessage_`+`_notifMakeRec_` dùng nó cho Message + cột DueDate; NEW `_notifReconcileDue_(sheet,cands)` thay `_notifRetractStale_` trong `notifScan` — **làm tươi** ngày khi deadline đổi (vẫn cùng ngưỡng) + **thu hồi** khi không còn candidate (deadline dời/done/mất), bao trùm done/missing cũ + bỏ 1 lượt đọc lại 5 sheet. `verify_notif_retract` **41/41** (+NR11–14), `verify_notifications` **21/21** (0 hồi quy). **✅ [TT] đã redeploy GAS (link KHÔNG đổi).** Nghiệm thu: kỳ digest kế + chạy tay `notifScan()` dọn tồn kho. Xem TD-NOTIF-03.

> **S78 (CR Kanban "Công việc của tôi" — thuần FE, v6.52)**: 3 yêu cầu trên view Kanban của My Work. (1) **Cột "Cần thực hiện"** gộp đúng 4 trạng thái `Chưa bắt đầu · Hoàn thành chuẩn bị · Tạm dừng · Blocked` — constant tường minh `MW_KB_TODO_STATES`, giữ negative-filter (`!inProc && !done`) nên không mất task lạ. (2) **Scroll đồng nhất**: cả 3 cột (To-do/In-process/Closed) dùng chung khung cuộn `max-height:520px` (`.mw-kb-col-body-scroll`), trước chỉ cột Closed. (3) **Filter theo nhân sự** cho Teamlead/Admin: droplist "Lọc theo nhân sự" (distinct picRes/picAcc trong scope) → lọc task người đó là Res/Acc, áp cả List+Kanban qua 1 nguồn `_mwScopedTasks`; đổi team reset nhân sự; User thường ẩn droplist; person filter in-memory (không persist). `assets/js/views/my-work.js`+`my-work.css`+`i18n.js`(+2 key VI/EN)+`config.js`+`index.html`(?v=20260822, 65 refs). `verify_my_work` **91/91** (+KB8/KB9/PF1–PF5). **KHÔNG cần redeploy GAS** — chỉ hard-reload. Commit `dbc84ce`.
...
> **S76 (My Work scope + Kanban + email nhắc việc — FEATURE)**: 3 việc độc lập. (1) **Loại email NHẮC VIỆC (digest) cho CuongVM1** (giữ email báo cáo định kỳ + chuông): `backend/NotificationService.gs` NEW `_notifDigestSuppressSet_()` đọc `Report_Config.Digest_Suppress` (fallback `['cuongvm1']`); `_notifSendDigests_` bỏ gửi user bị suppress nhưng VẪN mark EmailedDate. **Cần redeploy GAS.** (2) **My Work chỉ task CÁ NHÂN cho user thường** (`views/my-work.js`): NEW `_mwScopedTasks(user,teamFilter)` — User=`picRes∪picAcc`; **Teamlead=full team** (giữ cũ); **Admin=full trung tâm** + droplist team (mặc định team Admin). Bỏ mệnh đề `t.team===uteam` áp-mọi-role. Thuần FE. (3) **View KANBAN cho My Work**: toggle List⇄Kanban (persist), 3 cột To-do (quá hạn/gần deadline nhất) · In-process (count + **banner đỏ FTE quá tải**, đếm theo picRes ≥3 → badge đỏ từng card) · Closed (Deadline desc = proxy "đóng gần nhất", cap 15). CSS + i18n +11 key VI/EN. `verify_my_work` **78/78** (thêm MW40/41 role-scope + KB1–6 kanban), `verify_notif_retract` **24/24** (thêm NR10 digest-suppress GAS thật), full suite **35/36** (chỉ `issue_tracker` flaky batch, standalone 61/61). v6.50, `?v=20260821`. **Item 1 chờ redeploy GAS.**
