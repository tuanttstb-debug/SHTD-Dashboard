...
> **Delta 2026-09-04 (S84 — AI Assistant resilience Nhóm 1):** ✅ **Đóng gốc lỗi "high demand"** (503 quá tải không được retry ở tầng nào — `callGemini` throw thẳng, Web App bọc HTTP 200 nên FE `_aiPostWithRetry` không bắt). Nay có retry+backoff+fallback+degradation (deploy-gated: **[TT] redeploy GAS** mới hiệu lực). **Nợ Nhóm 2 còn mở (đưa vào TODO, không chặn):** ⚪ **TD-AI-01** — payload ~150-200K token/câu hỏi (nhồi index toàn bộ ~1500 task + 200 rich + KPI + initiative mỗi request) → latency ~15-20s + chi phí token + rủi ro 429/timeout; giảm bằng RAG/retrieval (KHÔNG chữa 503 nhưng chữa latency/cost). ⚪ **TD-AI-02** — implicit context caching hiện không ăn (prefix mở đầu bằng "Ngày hôm nay" đổi mỗi lần) → sắp phần tĩnh (system prompt + index) lên đầu, biến thiên xuống cuối. ⚪ **TD-AI-03** — gọi Gemini đồng bộ trong trần GAS 6' + FE timeout 120s có thể lệch; hướng async job. ⚪ **TD-AI-04** — chưa rate-limit per-user + chưa idempotency reqId cho ai-chat (retry transport FE có thể gọi Gemini trùng = đốt quota). ⚪ **TD-AI-05** — chuỗi model dự phòng cần `refreshAiModelChain()` chạy định kỳ (trigger) để bắt kịp khi Google shutdown model; hiện chạy tay. Data-boundary: 0 chạm KH/secret; degradation trong biên (không bên thứ 3). Lưu ý: `verify_mobile`/`verify_kpi_views` cần server ngoài 53704 (pre-existing).

> **Delta 2026-09-04 (S83 — CR Left-menu 2 lớp):** Không nợ CRITICAL/HIGH mới. Thuần FE + additive (menu 2 lớp gom nhóm). **Tồn/nợ nhẹ (không chặn):** ⚪ **TD-NAV-01** — badge dồn nhóm mẹ chỉ hiện **chấm đỏ** theo child `.nav-badge.danger` (Phê duyệt BLĐ/Issue breach/Overdue); badge **đếm thuần** (Task total/Case count) KHÔNG dồn (chủ đích tránh nhiễu số lớn thường trực). ⚪ **TD-NAV-02** — mở/gấp nhóm **chưa có animation** (đổi display tức thì); thêm max-height transition nếu [TT] muốn mượt. ⚪ **TD-NAV-TEST-01** — 9 test click nav item trong nhóm gấp đã vá bằng "mở nhóm cha trước click" (`.closest('.nav-group').classList.add('open')`); test mới nên dùng `navigateTo()` qua evaluate hoặc mở nhóm trước để tránh "element not visible". ⚪ Ghi chú: `verify_mobile`/`verify_kpi_views` là dev-harness cần server ngoài port 53704 (không tự chạy) — không liên quan CR này; `verify_initiative*` flaky file:// pre-existing. Data-boundary: 0 chạm dữ liệu KH/secret.

> **Delta 2026-09-01 (S82 — Calendar Sync Pha 1, TuanTT4):** Không nợ CRITICAL/HIGH mới. Tính năng CÔ LẬP (whitelist cứng TuanTT4, route mới, 0 đổi schema/luồng cũ). **Tồn/nợ nhẹ (đã đưa vào TODO, không chặn):** ⚪ **TD-CAL-01** — CalendarApp thật **chưa test tự động** (sandbox chỉ phủ logic thuần 32/32); đúng-sai create/update/delete event dựa [TT] smoke live. ⚪ **TD-CAL-02** — reconcile chạy theo **trigger @7h30 + thao tác thủ công** (Kết nối/Đồng bộ lại), CHƯA tức thì sau mỗi write → task đổi trong ngày phải "Đồng bộ lại" hoặc đợi hôm sau (Pha 2). ⚪ **TD-CAL-03** — "update" event = **xóa+tạo lại** (đơn giản, tránh sửa series) → eventId đổi mỗi lần nội dung đổi (vô hại, MAP tự cập nhật). ⚪ **TD-CAL-04** — event định kỳ (direct) CHƯA đặt popup reminder tùy biến (dựa mặc định lịch); event deadline direct có popup ~trưa hôm trước. ⚪ **TD-CAL-05** — quota CalendarApp (owner Gmail thường): giảm nhờ sync tăng dần (chỉ phần chênh); nếu tương lai bật multi-user cần chunk. Data-boundary: chỉ việc TuanTT4 lên Gmail TuanTT4; `CAL_SYNC_MAP` chỉ id nội bộ (0 PII bên thứ ba).

> **Delta 2026-08-31 ([TT] redeploy GAS + smoke test XONG → đóng deploy-gated):** [TT] đã **redeploy GAS toàn bộ + smoke test OK**. Các nợ vốn chỉ chờ redeploy nay **hiệu lực trên live** → coi như ĐÓNG phần deploy-gated: ✅ **Pha A** (idempotency `reqId` + retry + rút khóa `atomicUpsert_`) hiệu lực → ghi ~0% timeout/mất · ✅ **WRITE-PERF-B** (version-theo-domain) hiệu lực → batch-read chỉ tải domain đổi · ✅ **Keep-warm** opt-in sẵn sàng ([TT] gắn trigger tùy chọn). **Còn mở (KHÔNG phải deploy-gated — quyết định/cosmetic, không đóng được bằng deploy):** ⚪ **AUDIT-DOUBLEBUMP** (version bump 2 lần trong+ngoài khóa — vô hại; cần tách bump khỏi `auditLog` mới dọn) · **WRITE-PERF-C** đã **quyết KHÔNG làm** (đánh đổi trễ notif không đáng) · các TD-RECUR/TD-MW/TD-DATE/TD-NOTIF-03 là LOW/tùy chọn thiết kế. **Lưu ý bảo trì giữ nguyên:** mutation MỚI vào 1 domain PHẢI gọi `_bumpDomainVer('<domain>')`.
> **Delta 2026-08-26 (Pha B+C):** ✅ **WRITE-PERF-B ĐÓNG** — version theo domain (`SHTD_VER_<domain>`): batch-read chỉ tải lại domain đổi (sửa 1 task không kéo 6 domain kia). ✅ **Keep-warm** opt-in (`keepWarm()`, [TT] gắn trigger). **WRITE-PERF-C** (dồn notif sang trigger) — **QUYẾT ĐỊNH KHÔNG làm**: notifOnWrite đã ngoài lock (Pha A) + rẻ cho update thường (chỉ append khi create/close); gỡ hẳn = trễ notif tạo/đóng, đánh đổi không đáng. **Lưu ý bảo trì:** mọi mutation MỚI vào 1 domain PHẢI gọi `_bumpDomainVer('<domain>')` — nếu quên, client không thấy thay đổi domain đó (notifs bám global nên miễn). **AUDIT-DOUBLEBUMP** vẫn mở (vô hại). **[TT] cần redeploy GAS.**
