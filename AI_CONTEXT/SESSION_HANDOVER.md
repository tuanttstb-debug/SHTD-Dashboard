...
- **Task completed:** Rà soát tổng thể + test live production tính năng **AI Assistant** (lỗi thường gặp "Gemini API lỗi: This model is currently experiencing high demand" = HTTP 503 quá tải tạm thời). Chẩn đoán qua **2 agent** (nghiên cứu diễn đàn giải pháp + đào sâu kiến trúc) + **phản biện chéo**. **Gốc:** `callGemini` gọi Gemini 1 phát, gặp `json.error` throw thẳng; Web App bọc lỗi vào HTTP **200** `{status:'error'}` → FE `_aiPostWithRetry` chỉ retry exception HTTP 4xx/5xx nên KHÔNG bắt được → user thấy lỗi ngay dù 503 chỉ cần thử lại 1-2s. Triển khai trọn **Nhóm 1**: retry+backoff server-side, fallback model chain, bắt 404 riêng, lỗi có cấu trúc + nút Thử lại, degradation deterministic, log.
- **Files changed:** *(spoke, chờ commit phiên này)* `backend/AiService.gs` (viết lại `callGemini` + MỚI `_aiModelChain`/`_aiBackoffMs_`/`_aiErrClass_`/`_aiLog_`/`_aiSummaryFromContext_`/`_aiErrorResponse_`/`refreshAiModelChain`; maxOutputTokens 2048→4096), `backend/Code.gs` (route ai-chat try/catch → `_aiErrorResponse_`), `assets/js/views/ai-chat.js` (sendAiMessage xử lý lỗi có cấu trúc + `_aiRetry` + nút Thử lại), `assets/js/i18n.js` (+6 key `ai.err.*`/`ai.retry-btn`/`ai.degraded.header` VI/EN), `assets/css/ai-chat.css` (+`.ai-retry-btn`), `assets/js/config.js` (v6.59), `index.html` (cache-bust ai-chat.js/i18n.js/config.js/ai-chat.css → `?v=20260904b`), MỚI `test/verify_ai_resilience.mjs`.
- **Decision made:** Chuỗi model **khám phá động** (`refreshAiModelChain()` gọi models.list, chạy tay/trigger → Script Property, KHÔNG inline runtime) thay vì hard-code — **KHÔNG pin `gemini-2.5-flash`** (đã bị Google chặn với key/project này) và tránh model đã shutdown. Primary giữ alias `gemini-flash-latest`. Ngân sách retry **≤4-5 lời gọi Gemini/câu** (retry ăn quota RPD/RPM). **FE bỏ auto-retry lỗi Gemini** (server đã retry) → nút Thử lại thủ công. Lưới cuối = **degradation deterministic nội bộ** (số liệu tính sẵn), **KHÔNG OpenRouter/bên thứ 3** vì data-boundary ngân hàng (context chứa toàn bộ task/PIC/KPI nội bộ). Giảm payload (N2.1) tách sang Nhóm 2 — chữa latency/cost/429, KHÔNG chữa 503.
- **Blocker:** **Không.** **⚠️ Fix THẬT nằm ở backend → [TT] cần redeploy GAS** (AiService.gs + Code.gs) mới có hiệu lực retry+fallback. FE tự cập nhật qua Pages + hard-refresh (tương thích ngược GAS cũ, chỉ chưa có retry server).
- **Next step:** [TT] (P0) **redeploy GAS** + smoke: gửi câu hỏi lúc Gemini bận → không còn lỗi cứng (tự retry/fallback); nếu vẫn bận thì thấy thông điệp thân thiện + nút Thử lại + số liệu tính sẵn. (P1 tùy chọn) chạy `refreshAiModelChain()` 1 lần trong editor để bơm chain dự phòng (đọc kết quả trong Logger/AI_Log); **bật billing** giảm tần suất 503. (P2) hard-refresh `?v=20260904b`. [CC] Nhóm 2 khi [TT] muốn: giảm payload/RAG + sắp prefix tĩnh (ăn implicit cache) · async job bỏ trần 6' · observability + rate-limit · idempotency reqId cho ai-chat.
- **Regression risk:** **Thấp → verify đầy đủ.** `verify_ai_resilience` 42/42 + regression my_work 97/97 · startup_nonblocking 10/10 · nav_group 14/14 · parse OK 2 .gs. Thay đổi cô lập tầng AI: `callGemini` giữ contract trả string khi thành công; chuỗi model rỗng → chỉ primary (hành vi cũ); FE/GAS tương thích ngược 2 chiều. `_aiLog_` best-effort try/catch không phá luồng. Data-boundary: degradation trong biên, không bên thứ 3. Chưa smoke live phần retry thật (cần [TT] redeploy).

# SESSION HANDOVER — 2026-09-04 (S83 — CR Left-menu 2 lớp: gom nhóm menu mẹ, thu gọn sidebar)
**Model**: Claude Opus 4.8 · **Version**: v6.57.1 → **v6.58**

- **Task completed:** CR [TT] — left menu 28 dòng quá dài, gom tính năng cùng nhóm vào **menu mẹ gấp/mở**, giữ mục dùng nhiều phẳng ở trên. Đánh giá code (nav tĩnh + `navigateTo` độc lập DOM + collapse 68px + RBAC CSS) → thiết kế 2 lớp: 3 mục phẳng (My Work·Dev-plan·Dashboard) + **5 nhóm mẹ** (Ban lãnh đạo·Quản trị H2·Quản lý công việc·KPI Digital & Phân tích·Quản trị) + AI Assistant phẳng. Gấp hết ≈ 28→10 dòng. Badge dồn (chấm đỏ) ở nhóm mẹ khi gấp; flyout hover khi sidebar 68px; nhớ trạng thái qua localStorage; auto-mở nhóm chứa view active.
- **Files changed:** *(spoke, chờ commit phiên này)* MỚI `test/verify_nav_group.mjs`; sửa `index.html` (nav + 6 cache-bust `?v=20260904`), `assets/css/layout.css` (+nav-group/flyout/dot), `assets/js/ui/navigation.js` (+4 hàm group + wire), `assets/js/i18n.js` (+5 `nav.group.*` VI/EN, rút gọn nhãn H2), `assets/js/app.js` + `assets/js/views/issue-tracker.js` (+gọi updateNavGroupBadges), `assets/js/config.js` (v6.58). **Vá 9 test** click nav trong nhóm gấp (mở nhóm cha trước click): verify_bld_queue/h2_dashboard/h2_review/h2_tracker/case_pipeline + helper filter_cascade/import_rbac/initiative/initiative_v2/modal_layout.
- **Decision made:** [TT] chốt qua 3 câu hỏi: (1) **5 nhóm mẹ** (giữ phẳng My Work/Dev-plan/Dashboard/AI); (2) mặc định **mở "Quản lý công việc" + nhóm active**, còn lại gấp; (3) sidebar 68px → **flyout hover popup**. Badge dồn chỉ chấm đỏ theo child `.nav-badge.danger` (bỏ badge đếm thuần để tránh nhiễu). Bỏ 6 section-label phẳng cũ; rút gọn nhãn 3 mục con H2 (bỏ tiền tố lặp). Giữ nguyên phím tắt g+chữ, RBAC, mọi data-view (22/22 không mất view).
- **Blocker:** **Không.** Thuần FE — KHÔNG cần redeploy GAS.
- **Next step:** [TT] hard-refresh (`?v=20260904`) → nghiệm thu: gấp/mở nhóm nhớ trạng thái; sidebar thu gọn Ctrl+B → hover nhóm ra flyout; chấm đỏ ở nhóm gấp khi có việc chờ; mobile mở sidebar bung nhóm inline. [CC] (tùy chọn) thêm hiệu ứng slide khi mở/gấp; cho phép [TT] tự sắp thứ tự nhóm nếu cần.
- **Regression risk:** **Thấp → verify đầy đủ.** verify_nav_group 14/14 + mobile 375px PASS + regression rộng xanh (my_work 97/97·case_pipeline 22/22·bld_queue 45/45·h2 ×3·i18n_p8·issue 61/61·action_plan·es·modal·filter_cascade·atomic·notif·recurring·startup·calendar_sync). Thay đổi thuần FE + additive; badge dồn guard `typeof`; không đổi schema/route/luồng view. Pre-existing (không phải regression): verify_mobile/verify_kpi_views cần server ngoài 53704; verify_initiative* flaky file:// (retry timeout qua nav ở bước khác — initiative-tracker thuộc nhóm mở sẵn). Data-boundary: 0 chạm dữ liệu KH/secret.

# SESSION HANDOVER — 2026-09-01 (S82 — Calendar Sync "Công việc của tôi", Pha 1, TuanTT4)
**Model**: Claude Opus 4.8 · **Version**: v6.56 → **v6.57**

- **Task completed:** Xây tính năng **đồng bộ Task/Initiative lên Google Calendar** để nhắc việc, đặt ở "Công việc của tôi" — **1 công tắc tổng opt-in**, gỡ bất kỳ lúc nào. [TT] **thu hẹp scope → chỉ TuanTT4** (task của anh, Gmail của anh → data-boundary an toàn). Phạm vi: task định kỳ→event lặp · task/initiative đến hạn (≥ hôm nay, chưa xong)→event 1 lần; chỉ việc user phụ trách (Res/Acc). **Tự phát hiện** email đích == owner GAS → ghi thẳng lịch mặc định (nhắc chuẩn); khác → guest-invite lịch phụ. Reconcile idempotent qua sheet `CAL_SYNC_MAP` (create/update/delete phần chênh). Trạng thái ở Script Properties.
- **Files changed:** *(spoke, chờ commit phiên này)* MỚI `backend/CalendarSyncService.gs`, `test/verify_calendar_sync.mjs`, `docs/PROPOSAL_Calendar_Sync_2026-09-01.md`; sửa `backend/Code.gs` (+3 route cal-*), `assets/js/api.js` (+3 hàm apiCal*), `assets/js/views/my-work.js` (+thẻ 🔔 + handlers), `assets/js/i18n.js` (+19 key VI/EN), `assets/css/my-work.css` (+card theme-aware), `assets/js/config.js` (v6.57), `index.html` (cache-bust 5 file `?v=20260901`).
- **Decision made:** [TT] thu hẹp chỉ TuanTT4 (bỏ cột User_Master + toggle Quản trị người dùng → thừa với 1 user; lưu state Script Properties). Tiêu đề event = đầy đủ (option A). Ghi lịch tự phát hiện direct-vs-guest (owner==gmail → direct, [TT] xác nhận DB gmail = gmail chính = owner). Update = xóa+tạo lại (tránh sửa series phức tạp). Chỉ sync việc chưa xong + hạn ≥ hôm nay (quá hạn để in-app lo). Service tự chứa helper (parse/recur riêng) để test sandbox độc lập, không nạp NotificationService.
- **Blocker:** **Không.** **✅ [TT] đã redeploy GAS** (dán CalendarSyncService.gs + Code.gs, Authorize Calendar, chạy `installCalendarSyncTrigger()`, link KHÔNG đổi).
- **Next step:** [TT] hard-refresh → "Công việc của tôi" → nhập Gmail → **Kết nối** → mở Google Calendar kiểm event (định kỳ lặp + deadline); tick xong/đổi deadline → chạy "Đồng bộ lại" hoặc chờ trigger @7h30 thấy cập nhật; **Ngắt kết nối** → event biến mất. [CC] (tùy chọn) Pha 2: reconcile tức thì sau write; thêm popup reminder cho event định kỳ; mở rộng multi-user (thiết kế §5–§7 proposal).
- **Regression risk:** **Thấp → verify `verify_calendar_sync` 32/32 + my_work 97/97 (0 JS error).** Tính năng CÔ LẬP: whitelist cứng TuanTT4 (2 tầng: service + FE ẩn card), route mới không đụng luồng cũ, không đổi schema Task/Initiative/User_Master, không bump domain version. CalendarApp thật chưa test tự động (cần tài khoản Google) → [TT] smoke live. Data-boundary: chỉ việc của TuanTT4 lên Gmail của TuanTT4; `CAL_SYNC_MAP` chỉ id nội bộ.
