...
async function bulkSetRag(rag) {
  ...
  if (!ok) return;
  const synced = await syncAction(() => { selectedIds.forEach(id => { const t = db.tasks.find(x=>x.id===id); if(t) t.status = rag; }); });
  if (!synced) return;
...
async function bulkSetState(state) {
  ...
  const synced = await syncAction(() => { selectedIds.forEach(id => { const t = db.tasks.find(x=>x.id===id); if(t) { t.state=state; if(state==='Hoàn thành') t.progress=100; } }); });
  if (!synced) return;
...
async function bulkDelete() {
  ...
  if (!ok) return;
  const count = selectedIds.size;
  const synced = await syncAction(() => { db.tasks = db.tasks.filter(t => !selectedIds.has(t.id)); });
  if (!synced) return;
