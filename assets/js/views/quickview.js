...
function _qvPopulateFilters() {
  ...
  initSel.innerHTML = `<option value="">${t('qv.filter.all')}</option>`
...
function renderQuickView() {
  ...
  document.getElementById('qvpSubtitle').textContent = subParts.length
    ? subParts.join(' · ') + ` · ${tasks.length} ${t('task.count.unit')}`
    : `${db.tasks.length} ${t('task.count.unit')} · ${t('qv.subtitle.all')}`;
...
function _qvRenderEmpty() {
  const msg = `<div class="qvp-empty">
    ...
    <div class="qvp-empty-title">${t('qv.empty.title')}</div>
    <div class="qvp-empty-sub">${t('qv.empty.sub')}</div>
  </div>`;
...
function _qvUpdateTime() {
  ...
  if (el) el.textContent = t('qv.time-prefix') + ' ' + new Date().toLocaleTimeString('vi-VN',{hour:'2-digit',minute:'2-digit',second:'2-digit'});
}
...
function _qvRenderDone(tasks) {
  ...
  if (!done.length) {
    document.getElementById('qvDoneList').innerHTML = `<div class="qvp-empty">
      ...
      <div class="qvp-empty-title">${t('qv.done.empty-title')}</div>
      <div class="qvp-empty-sub">${t('qv.done.empty-sub')}</div>
    </div>`;
    return;
  }

  document.getElementById('qvDoneList').innerHTML = done.map(tk => `
    <div class="qvp-card qvp-done-card ${_qvRagCls(tk.status)}" onclick="_qvOpenTask('${tk.id}')">
      <div class="qvp-card-top">
        <div style="display:flex;align-items:flex-start;gap:8px;flex:1;min-width:0;">
          <div class="qvp-done-check"><i class="fa-solid fa-check"></i></div>
          <div style="flex:1;min-width:0;">
            <div class="qvp-card-id">${esc(tk.id)}</div>
            <div class="qvp-card-name">${esc(tk.name)}</div>
          </div>
        </div>
        <div class="qvp-card-badges">${_qvRagBadge(tk.status)}</div>
      </div>
      <div class="qvp-card-meta">
        ${tk.initiative ? `<span class="qvp-card-meta-item"><i class="fa-solid fa-layer-group"></i>${esc(tk.initiative)}</span>` : ''}
      ...
      ${(tk.result||'').trim() ? `
        <div style="margin-top:8px;padding-top:8px;border-top:1px dashed var(--border);font-size:12px;color:var(--text-2);line-height:1.5;">
          <i class="fa-solid fa-quote-left" style="opacity:.3;margin-right:4px;font-size:10px;"></i>${esc(tk.result)}
        </div>` : ''}
      <div class="qvp-prog">
...
function _qvRenderPlan(tasks) {
  ...
  if (!plan.length) {
    document.getElementById('qvPlanList').innerHTML = `<div class="qvp-empty">
      ...
      <div class="qvp-empty-title">${t('qv.plan.empty-title')}</div>
      <div class="qvp-empty-sub">${t('qv.plan.empty-sub')}</div>
    </div>`;
    return;
  }
  ...
  document.getElementById('qvPlanList').innerHTML = `
    <div style="display:flex;align-items:center;gap:8px;margin-bottom:14px;">
      ...
      <span style="font-size:12px;color:var(--text-3);">${plan.length} ${t('qv.plan.work-count')}</span>
    </div>
    ${plan.map(tk => {
      const inNextMonth = tk.endDate && (() => {
        const d = new Date(tk.endDate);
        return d >= nextM && d <= nextMEnd;
      })();
      return `
      <div class="qvp-card qvp-plan-card ${_qvRagCls(tk.status)}" onclick="_qvOpenTask('${tk.id}')">
        <div class="qvp-card-top">
          <div style="flex:1;min-width:0;">
            <div class="qvp-card-id">${esc(tk.id)}</div>
            <div class="qvp-card-name">${esc(tk.name)}</div>
          ...
          <div class="qvp-card-badges" style="flex-direction:column;align-items:flex-end;gap:4px;">
            ${_qvStateChip(tk.state)}
            ${inNextMonth ? `<span class="qvp-deadline-tag"><i class="fa-solid fa-flag"></i>${t('qv.plan.deadline-tag')}</span>` : ''}
          </div>
        </div>
        <div class="qvp-card-meta">
          ${tk.initiative ? `<span class="qvp-card-meta-item"><i class="fa-solid fa-layer-group"></i>${esc(tk.initiative)}</span>` : ''}
          ${tk.team ? `<span class="qvp-card-meta-item"><i class="fa-solid fa-users"></i>${esc(tk.team)}</span>` : ''}
          ${tk.picRes ? `<span class="qvp-card-meta-item"><i class="fa-solid fa-user"></i>${esc(tk.picRes)}</span>` : ''}
          ${tk.endDate ? `<span class="qvp-card-meta-item"><i class="fa-solid fa-calendar"></i>${t('qv.deadline-prefix')} ${_qvFmtDate(tk.endDate)}</span>` : ''}
        </div>
        ${(tk.nextPlan||'').trim() ? `
          <div class="qvp-plan-content">
            <span style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.05em;color:var(--accent);display:block;margin-bottom:4px;">${t('qv.plan.next-label')}</span>
            ${esc(tk.nextPlan)}
          </div>` : ''}
        <div class="qvp-prog">
          <div class="qvp-prog-bar">
            <div class="qvp-prog-fill" style="width:${tk.progress}%;"></div>
          </div>
          <span class="qvp-prog-pct">${tk.progress}%</span>
        </div>
      </div>`;
    }).join('')}`;
}
...
function _qvRenderInitiative(tasks, filterInit) {
  ...
  if (!inProg.length) {
    document.getElementById('qvInitList').innerHTML = `<div class="qvp-empty">
      ...
      <div class="qvp-empty-title">${t('qv.init.empty-title')}</div>
      <div class="qvp-empty-sub">${t('qv.init.empty-sub')}</div>
    </div>`;
    return;
  }
  ...
  if (!singleMode) {
    html += `<div class="qvp-section-title" style="margin-bottom:0;">
      <i class="fa-solid fa-layer-group" style="color:var(--primary);"></i> ${t('qv.init.group-by')}
    </div>`;
  }
        ...
        <div class="qvp-card ${_qvRagCls(tk.status)}" onclick="_qvOpenTask('${tk.id}')" style="margin-left:${singleMode?0:12}px;">
          ...
          <div class="qvp-card-meta">
            ...
            <span class="qvp-card-meta-item" ${isOverdueTask?'style="color:var(--danger);font-weight:700;"':''}>
              ...
              ${isOverdueTask ? t('qv.overdue-prefix') : t('qv.deadline-prefix')} ${_qvFmtDate(tk.endDate)}
            </span>
            ${tk.milestone ? `<span class="qvp-card-meta-item" style="background:var(--primary-xlight);padding:1px 6px;border-radius:3px;color:var(--primary);font-weight:700;"><i class="fa-solid fa-flag-checkered"></i>${esc(tk.milestone)}</span>` : ''}
          </div>
          <div class="qvp-prog">
            <div class="qvp-prog-bar"><div class="qvp-prog-fill" style="width:${tk.progress}%;"></div></div>
            <span class="qvp-prog-pct">${tk.progress}%</span>
          </div>
        </div>`;
      }).join('');
...
function _qvRenderIssue(tasks) {
  ...
  if (!issues.length) {
    document.getElementById('qvIssueList').innerHTML = `<div class="qvp-empty">
      ...
      <div class="qvp-empty-title">${t('qv.issue.empty-title')}</div>
      <div class="qvp-empty-sub">${t('qv.issue.empty-sub')}</div>
    </div>`;
    return;
  }

  document.getElementById('qvIssueList').innerHTML = issues.map(tk => {
    const flags = [];
    if (tk.state === 'Blocked') flags.push(`<span class="qvp-issue-flag blocked"><i class="fa-solid fa-ban"></i> Blocked</span>`);
    if (tk.canBLD === 'Y')      flags.push(`<span class="qvp-issue-flag bld"><i class="fa-solid fa-bell"></i> ${t('qv.issue.flag-bld')}</span>`);
    if ((tk.vuongMac||'').trim() && tk.state !== 'Blocked') flags.push(`<span class="qvp-issue-flag vuong"><i class="fa-solid fa-circle-exclamation"></i> ${t('qv.issue.flag-vuong')}</span>`);

    return `
    <div class="qvp-card qvp-issue-card rag-issue" onclick="_qvOpenTask('${tk.id}')">
      <div class="qvp-card-top">
        <div style="flex:1;min-width:0;">
          <div class="qvp-card-id">${esc(tk.id)}</div>
          <div class="qvp-card-name">${esc(tk.name)}</div>
      ...
      ${(tk.vuongMac||'').trim() ? `
        <div class="qvp-issue-text">
          <span style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.05em;color:var(--info);display:block;margin-bottom:4px;">
            <i class="fa-solid fa-triangle-exclamation" style="margin-right:3px;"></i>${t('qv.issue.risk-title')}
          </span>
          ${esc(tk.vuongMac)}
        </div>` : ''}
      ${(tk.noiDungBLD||'').trim() ? `
        <div class="qvp-issue-text" style="border-top-color:var(--warning);margin-top:${(tk.vuongMac||'').trim()?'8px':'0'};">
          <span style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.05em;color:var(--warning);display:block;margin-bottom:4px;">
            <i class="fa-solid fa-bell" style="margin-right:3px;"></i>${t('qv.issue.bld-title')}
          </span>
          ${esc(tk.noiDungBLD)}
        </div>` : ''}
      ${(tk.yKienBLD||'').trim() ? `
        <div class="qvp-issue-text" style="border-top-color:var(--info);margin-top:8px;">
          <span style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.05em;color:var(--info);display:block;margin-bottom:4px;">
            <i class="fa-solid fa-comment-dots" style="margin-right:3px;"></i>${t('qv.issue.bld-opinion')}
          </span>
          ${esc(tk.yKienBLD)}
        </div>` : ''}
      <div class="qvp-prog" style="margin-top:10px;">
        <div class="qvp-prog-bar"><div class="qvp-prog-fill" style="width:${tk.progress}%;background:${tk.state==='Blocked'?'var(--danger)':'var(--warning)'};"></div></div>
        <span class="qvp-prog-pct">${tk.progress}%</span>
      </div>
    </div>`;
  }).join('');
